...
@section('title','Edición de Tipo de cartilla /')
...
@section('content-page')
	<div class="row">
		<div class="col-md-12 col-sm-12 col-xs-12">
			<div class="x_panel">
				<div class="x_title">
					<h2>Editar <small>Tipo de cartilla</small></h2>
					<div class="clearfix"></div>
				</div>
				<div class="x_content">
					<form method="post" class="form-horizontal form-label-left" action="{{ route('book-types.update',$bookType->id) }}">
						<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<input type="hidden" name="_method" value="PUT">
						<input type="hidden" name="key" value="{{$bookType->id}}">
						<div class="row">
							<div class="form-group col-md-4 col-sm-4 col-xs-12">
								<label class="control-label col-md-2 col-sm-2 col-xs-12">Nombre </label>
							     <div class="col-md-9 col-sm-9 col-xs-12 if @if($errors->has('name')) has-error @endif">
							     	<input type="text" class="form-control" placeholder="Nombre" name="name" value="{{ $bookType->name }}" autofocus>
							...
							<div class="form-group col-md-5 col-sm-5 col-xs-12 ">
								<label class="control-label col-md-2 col-sm-2 col-xs-12">Descipción </label>
							     <div class="col-md-10 col-sm-10 col-xs-12 @if($errors->has('description')) has-error @endif">
							     	<input type="text" class="form-control" placeholder="Descripción" name="description" value="{{ $bookType->description }}">
							...
							<div class="form-group col-md-3 col-sm-3 col-xs-12 ">
								<label class="control-label col-md-2 col-sm-2 col-xs-12">Precio </label>
							     <div class="col-md-9 col-sm-9 col-xs-12 @if($errors->has('price')) has-error @endif">
							     	<input type="number" class="form-control" placeholder="Precio" name="price" value="{{$bookType->price}}">
